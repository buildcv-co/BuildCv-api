...
| 022 | `llm-integration-local-mvp` (cross-repo: optional local-first LLM feedback) | v0.5.4 | 🚧 **EN CURSO — PR1 API MERGED** (`LlmFeedback` contracts/options + deterministic `FakeLlmFeedbackClient`; default `Enabled=false`, `Provider=fake`, `Model=fake-local-v1`; no endpoint yet; `/score`, `/adapt`, auth-web unchanged). Art. II preserved: score remains deterministic and `ScoringEngine.Version` stays `2.0.0`. Artifact home: [../BuildCv-web/specs/022-llm-integration-local-mvp/](../BuildCv-web/specs/022-llm-integration-local-mvp/). | `main` | `2.0.0` |
