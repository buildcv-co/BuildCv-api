...
| 017 | `subscription-followups` | v1 | 📋 PLANEADO (3 deferred WARNINGs from 016 verify) | — | — |
...
| 021 | `structured-cv-import-and-job-input` (cross-repo: web frontend + api backend) | v0.5.3 | 🚧 EN CURSO (feature-branch-chain, PR 6d final micro-batch shipped on `feature/021-structured-cv-import-and-job-input`; JSON Resume `CvDocument` + `ConfidenceMarker` en parsers; `JobSpec` mandatory con Zod (web) + FluentValidation (api); `ScoringEngine.Version` bump MAYOR `1.0.0 → 2.0.0` (Art. II SemVer seal); `perSection: {experience, education, skills, certifications, contact}` + `redFlags[]`; discriminated-union `ScoreCvCommand` con backward-compat shim v1; editor open-resume-inspired (`BasicsForm`/`WorkList`/`EducationList`/`SkillsByCategory`); `promoteConfidence` solo en editor on blur (Constitution Art. I); feature flag `NEXT_PUBLIC_STRUCTURED_INPUT=true` (default); 6 chained PRs on feature branch, ~21 work-unit commits; determinism property test (1000 iter + parallel byte-identical); pending sdd-verify → sdd-archive) | `feature/021-structured-cv-import-and-job-input` | `2.0.0` (bumped from `1.0.0`; v1 path retained via `engineVersion: "1.0.0"` for one release cycle) |
