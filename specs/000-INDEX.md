...
| 022 | `llm-integration-local-mvp` (cross-repo: optional local-first LLM feedback) | v0.5.4 | ✅ **SHIPPED + ARCHIVED** — verify `PASS_WITH_NOTES`; tag `022-llm-integration-local-mvp-v1.0` created at API SHIP commit `00f64ed` (not archive commit). API MVP scope: PR1 (`85c85de`, contracts `LlmFeedback` + port + `FakeLlmFeedbackClient` + options + tests) + PR2 (`00f64ed`, `POST /api/v1/llm/feedback` + PII redaction + sanitized logs + dedicated rate limit + timeout/provider degraded fallback + contract tests). Cross-repo scope also includes Web PR3+PR4. Art. II preserved: score remains deterministic and `ScoringEngine.Version` stays `2.0.0`. Archive report: [../BuildCv-web/specs/022-llm-integration-local-mvp/archive-report.md](../BuildCv-web/specs/022-llm-integration-local-mvp/archive-report.md). | `main` | `2.0.0` |
| 024 | `minimax-real-provider` (cross-repo: optional MiniMax cloud feedback provider) | v0.5.5 | ✅ **SHIPPED + ARCHIVED** — verify `PASS_WITH_NOTES`; tag `024-minimax-real-provider-v1.0` created at API SHIP commit `98995c3` (not archive commit). API MVP scope: PR1 (`8b83cba`, `MinimaxLlmFeedbackClient`, options `BaseUrl`/`ApiKey`/`Model`/caps, conditional DI, safe appsettings, 22 tests) + PR2 (`98995c3`, handler dispatch, `MaxInputLength`, `MaxOutputTokens`, sanitized error mapping, endpoint integration, 7 tests). Cross-repo scope also includes Web PR3 type-only contract fix (`1a0a563`). Art. II preserved: score remains deterministic and `ScoringEngine.Version` stays `2.0.0`. Archive report: [../BuildCv-web/specs/024-minimax-real-provider/archive-report.md](../BuildCv-web/specs/024-minimax-real-provider/archive-report.md). | `main` | `2.0.0` |
