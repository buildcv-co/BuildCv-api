...
| 022 | `llm-integration-local-mvp` (cross-repo: optional local-first LLM feedback) | v0.5.4 | 🚧 **EN CURSO — PR2 API APPLIED** (`POST /api/v1/llm/feedback` + PII redaction + sanitized logs + dedicated rate limit + timeout/provider degraded fallback; fake provider only; `/score`, `/adapt`, auth-web source unchanged). PR1 API merged at `85c85de`; PR2 branch: `feature/022-llm-local-pr2-api-feedback-endpoint`. See [../BuildCv-web/specs/022-llm-integration-local-mvp/apply-progress.md](../BuildCv-web/specs/022-llm-integration-local-mvp/apply-progress.md). Art. II preserved: score remains deterministic and `ScoringEngine.Version` stays `2.0.0`. Artifact home: [../BuildCv-web/specs/022-llm-integration-local-mvp/](../BuildCv-web/specs/022-llm-integration-local-mvp/). | `feature/022-llm-local-pr2-api-feedback-endpoint` | `2.0.0` |
