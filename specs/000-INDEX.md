...
| 024 | `minimax-real-provider` (cross-repo: optional MiniMax cloud feedback provider) | v0.5.5 | 🚧 **EN CURSO — PR2 API APPLIED**: MiniMax provider foundation from PR1 plus PR2 handler/endpoint integration: provider metadata preserved on degraded fallback, max-input validation before provider call, redaction toggle coverage, sanitized 502 for unavailable provider errors, 429 provider rate-limit mapping with optional `Retry-After`, minimax endpoint contract tests, and fake-provider regression intact. No web functional changes, no real provider calls, no secrets tracked. | `feature/024-minimax-pr2-api-dispatch` | `2.0.0` |
