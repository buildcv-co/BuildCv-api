...
> **Source delta**: [`BuildCv-web/specs/021-structured-cv-import-and-job-input/specs/005-cv-pdf-docx-import/spec.md`](../../BuildCv-web/specs/021-structured-cv-import-and-job-input/specs/005-cv-pdf-docx-import/spec.md)
...
The web client MUST support both shapes via `NEXT_PUBLIC_STRUCTURED_INPUT` flag (default `"false"` during the rollout window). When the flag is `off`, the client requests `engineVersion: "1.0.0"`; when `on`, it uses `2.0.0`.
