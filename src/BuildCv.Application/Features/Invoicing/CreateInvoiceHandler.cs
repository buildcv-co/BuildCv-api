...
public sealed class CreateInvoiceHandler(IInvoiceStore store)
{
    public async Task<Result<Invoice>> HandleAsync(CreateInvoiceCommand command, CancellationToken ct)
    {
        var referenceCode = $"BUILDCV-{Guid.NewGuid().ToString("N")[..8].ToUpper()}";

        var invoice = new Invoice
        {
            Id = Guid.NewGuid(),
            UserId = command.UserId,
            DocumentType = InvoiceType.Bill,
        ...
        await store.AddAsync(invoice, ct);
