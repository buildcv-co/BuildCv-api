...
| 022 | `llm-integration-local-mvp` (cross-repo: optional local-first LLM feedback) | v0.5.4 | 🚧 **EN CURSO — PR1 API APPLIED, MERGE BLOCKED** (`LlmFeedback` contracts/options + deterministic `FakeLlmFeedbackClient`; default `Enabled=false`, `Provider=fake`, `Model=fake-local-v1`; no endpoint yet; `/score`, `/adapt`, auth-web source unchanged). Merge blocked by required `/adapt` regression command failure; see [../BuildCv-web/specs/022-llm-integration-local-mvp/apply-progress.md](../BuildCv-web/specs/022-llm-integration-local-mvp/apply-progress.md). Art. II preserved: score remains deterministic and `ScoringEngine.Version` stays `2.0.0`. Artifact home: [../BuildCv-web/specs/022-llm-integration-local-mvp/](../BuildCv-web/specs/022-llm-integration-local-mvp/). | `feature/022-llm-local-pr1-api-fake-provider` | `2.0.0` |
