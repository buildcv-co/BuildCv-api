...
| 024 | `minimax-real-provider` (cross-repo: optional MiniMax cloud feedback provider) | v0.5.5 | 🚧 **EN CURSO — PR1 API APPLIED**: `MinimaxLlmFeedbackClient` foundation, `LlmFeedback` MiniMax options (`BaseUrl`, server-side `ApiKey`, `MaxInputLength`, `MaxOutputTokens`), conditional DI, safe appsettings defaults, and unit tests. No endpoint dispatch yet (PR2), no web functional changes, no real provider calls, no secrets tracked. | `feature/024-minimax-pr1-api-client` | `2.0.0` |
